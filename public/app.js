    ...
    <div className="container">
      ...
      {IS_ADMIN && tab === 'admin' && (
        <>
              ...
              <div className="grid-2">
                {brands.map(b => (
                  <div key={b.id} className="mix-card">
                    ...
                    <details>
                      <summary>Вкусы</summary>
                      {b.flavors.map(f => (
                        <div key={f.id} className="row between tiny">
                          <div>{f.name} ({f.taste})</div>
          ...
          <div className="card">
            <div className="hd"><h3>Добавить вкус</h3></div>
            <div className="bd">
              ...
              <input className="input" placeholder="Тип вкуса (сладкий, пряный...)" value={flavorTaste} onChange={e => setFlavorTaste(e.target.value)} />
              <div>Крепость: {flavorStrength}</div>
          ...
          <div className="card">
            ...
            <div className="bd">
              <div className="row">
                <input className="input" placeholder="Введите слово" value={banInput} onChange={e => setBanInput(e.target.value)} />
                <button className="btn" onClick={addBan}>Добавить</button>
              </div>
              <div className="sep"></div>
              {(banned || []).length === 0
                ? <div className="tiny muted">Пусто</div>
                : banned.map(w => (
                  <div key={w} className="row between tiny">
                    <div>{w}</div>
                    <button className="btn small danger" onClick={() => delBan(w)}>✕</button>
                  </div>
                ))}
            </div>
          </div>
